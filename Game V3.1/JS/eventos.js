...
function ajusta_cores(ajustadeira, funcao){
    ...
    }else if(funcao == 2){ // Barra de Especiais
        switch (ajustadeira){
            case 1:
            ...
            break;
            case 4:
                document.getElementById("qtdEspeciais").style.color = "greenyellow";
            break;
            default:
...
function preenche_barra(){
    cronometro = setInterval(function(){
        if(jogo.contador_tempo_interno >= 0 && (estadoAtual == estados.jogando || estadoAtual == estados.ocioso)){
            ...
            if(jogo.contador_tempo_interno <= 3 && jogo.contador_tempo_interno != 0 && estadoAtual == estados.jogando)
                if(jogo.contador_tempo_interno > 1)
