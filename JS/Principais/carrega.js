...
function carrega_jogo(){
    ...
    var carregar_departamentos = [sincronizaQualidadeGrafica(), main(), iniciaAnimacao(), aleatorizaProp(), sincronizaQuadroConquistas(), carrega_idioma(1)];
    ...
    if(ultima_sessao == null)
        ...
        if(data_atual != ultima_sessao)
            ...
            carregar_tudo = false; // false p/ sempre carregar
...
function carrega_sons(texto){
    ...
    var lista_final = ["morreu_1", "morreu_2", "morreu_3", "morreu_4", "morreu_5", "morreu_6", "morreu_7", "morreu_8", "morreu_9", "morreu_10", "morreu_11", "morreu_12", "morreu_13", "morte_agua", "morte_lava"];
    ...
    carregar_sons = setInterval(function(){

        if(indice == alvo.length){
            clearInterval(carregar_sons);
            audio.pause();
